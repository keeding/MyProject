...
<script type="text/javascript">
	...
	function doAssociations(){
		...
		if(row.length == 1){
			...
			$.post('${pageContext.request.contextPath }/decidedzoneAction_findnoassociationCustomers.action', {}, function(data){
				for(var i=0;i<data.length;i++){
					$("#noassociationSelect").append("<option value='" + data[i].id + "'>"+data[i].name+"</otpion>");
				}
			})
			$.post('${pageContext.request.contextPath }/decidedzoneAction_findhasassociationCustomers.action', {id:row[0].id}, function(data){
				for(var i=0; i<data.length; i++){
					$("#associationSelect").append("<option value='" + data[i].id + "'>"+data[i].name+"</otpion>");
				}
			})
