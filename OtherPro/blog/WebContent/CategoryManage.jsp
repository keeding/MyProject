		    ...
		    <table class="table">
		      ...
		      <c:forEach items="${cgList }" var="cg">		      
		        <tr>
		          ...
		          <c:otherwise>
		              <a href="<%=basePath %>category?action=edit&cgId=${cg.id}"><i class="glyphicon glyphicon-pencil"></i></a>
		              <a href="<%=basePath %>category?action=delete&cgId=${cg.id }&userId=${user.id}"><i class="glyphicon glyphicon-remove"></i></a>
			       </c:otherwise>   			
			      </c:choose>
		          </td>
		        </tr>
